...
### 2026-09-05 — "영화 상세" 검증 화면 피드백 4건 처리, 일반화 가능한 3건만 design.md에 반영
사용자가 도메인 일반화 검증 화면("영화 상세", `6767:474`)에 피드백 4건을 줬다. (1) App Bar 타이틀을 숨긴 게 참고 화면(상품 상세)의 예외 사례를 App Bar 자신의 Content 규칙보다 우선시한 판단 오류였음을 확인 — 타이틀 복원. (2) `Thumbnail`을 포스터에 안 쓴 이유를 재확인한 결과 전 사이즈가 정사각형 전용이라 2:3 세로 비율에 애초에 대응 불가능함을 재확인 — 의도된 선택이었음. (3) 한줄 소개를 영화 설명 섹션으로 이동 — 콘텐츠 배치(IA) 판단이라 design.md 원칙화는 하지 않음. (4) cast-item 간격이 넓어 보인다는 지적을 실측한 결과 `itemSpacing`(6px)은 토큰대로 정확히 적용돼 있었고, 원인은 텍스트 박스 자체의 150% 줄간격이 이미 여백을 포함해 spacing과 중첩되며 체감 여백이 커진 것 — `2`로 축소.

일반화 가능한 3건(1/2/4)만 design.md에 반영: §0 원칙 6번(참고 화면 예외보다 컴포넌트 목적 우선), §2.14 Thumbnail Don't 항목(정사각형 전용 한계), §1.3 Spacing 캐비어트(텍스트 줄간격과 spacing 토큰의 중첩 효과). 3번(콘텐츠 배치)은 화면 하나의 판단이라 일반 원칙화하지 않고 validation 파일에만 기록. → `design-system/validation/movie-detail-screen.md`, `design-system/design.md`(v0.31)

### 2026-09-05 — "장바구니" 검증 화면 피드백 3건 처리, 일반화 가능한 1건만 design.md에 반영
사용자가 "장바구니" 검증 화면에 피드백 3건을 줬다. (1) 카드 좌우 16px 여백이 사라져 있었음 — 원인은 화면 최상위 프레임에 padding 없이 `Item Card / Cart`를 `layoutSizingHorizontal=FILL`로 붙여 카드가 360px로 늘어난 것. 별도 `item-list` 컨테이너(padding 16)로 감싸 해결. (2) 아이템 사이 divider가 실제 화면(item-16px-divider-16px-item)과 달리 간격 없이 붙어 있었고, divider 자체도 좌측 정렬돼 있었음 — `item-list`의 `itemSpacing=16`으로 리듬을 재현하고 FILL 폭으로 정렬 문제도 해소. (3) 필수 요소는 다 들어갔지만 추천 상품 노출 같은 비즈니스 관점의 깊이가 부족하다는 지적 — 실제 화면에 이미 있는 `Item Card / Recommendation` 캐러셀 패턴을 가져와 "다시 담아보세요" 섹션 신규 추가.

일반화 가능한 1건(divider 폭 규칙: 풀블리드 360 vs 인셋 328)만 §6에 반영. 나머지 2건(오토레이아웃 FILL+padding 확인 습관, 제품 판단 관점)은 Figma 구현 기법·작업 습관에 가까워 design.md에는 반영하지 않고 validation 파일에만 기록. → `design-system/validation/cart-screen.md`, `design-system/design.md`(v0.32)

### 2026-09-05 — 비커머스 2차 검증 "개인 활동 대시보드" 진행, design.md는 A(커머스 설명 수준)에 더 가깝다고 결론
design.md의 일반화 가능성을 검증하는 세 번째 화면("개인 활동 대시보드" — 지표 카드, 바 차트, 상태가 다른 활동 리스트, Empty 상태)을 제작. `App Bar`/`Tab Group`(기간 선택, §4 원칙 10과 정확히 부합)/`Section Header`/`Thumbnail`/`Icon Chevron Right` 5개 컴포넌트를 재사용했고 새 컴포넌트는 0개 만들었다. 색상·타이포·spacing·radius는 전부 기존 토큰만 사용(새 값 0개), `Feedback/Success`=파랑 렌더링 규칙(v0.30)도 실제로 적용해 "완료" 상태를 파랑으로 처리(처음엔 실수로 초록을 썼다가 스스로 발견해 수정).

**design.md에 규칙이 없어 추론한 부분**: 차트 막대 색상(Price Block의 "무할인=Gray/900" 관례를 일반화해 사용, 확정 규칙 아님), KPI 숫자 표시 타이포그래피, 상태 pill 배경색, 카드형 컨테이너 스타일(1px stroke+radius medium — Elevation 원칙의 일반화로 §1.6에 반영). `List` 컴포넌트는 실제로 확인해보니 한 줄짜리라 타임스탬프·상태·이동 액션을 담지 못해 그대로 못 쓰고 `Thumbnail`+`Icon/Chevron Right`+raw 텍스트 조합으로 대체.

**종합 판단**: Foundation(색상·타이포·spacing·radius)과 화면구성 원칙은 세 번의 비커머스 검증(영화 상세, 장바구니, 이번 대시보드) 내내 한 번도 어기지 않고 그대로 통했다. 반면 §2 Component의 콘텐츠 담당 컴포넌트(Item Card 계열, Price Block, Spec Row, Label, Chip, Rocket Badge, Status Badge, Review 계열)는 세 번 다 단 하나도 재사용되지 못했다 — design.md는 "커머스와 무관한 일반적 디자인 언어(B)"보다는 아직 "특정 커머스 UI를 설명하는 수준(A)"에 더 가깝다고 결론지었다. §5에 신규 백로그 3건(Chart, Stat 카드, 비인터랙티브 상태/태그 컴포넌트) 추가, §1.6에 카드형 컨테이너 구성 원칙 추가. → `design-system/validation/activity-dashboard.md`, `design-system/design.md`(v0.33)

### 2026-09-06 — 외부 디자인 시스템(Airbnb) 비교 감사, Text Input 공백을 최우선 보완 항목으로 지목
design.md 전체를 외부 기업 디자인 시스템 문서(`design-references/airbnb-design.md`)와 대조해 문서화 공백을 진단(외부 스타일·토큰은 이식하지 않고 "비교 기준"으로만 사용). 15개 Component Checklist 어디에도 Text Input/Form Field가 없다는 것을 "실제 UI 품질에 영향을 줄 정도로 반드시 보완 필요"의 1순위로 결론지었다. 그 외 터치 타겟 최소 크기·인터랙션 타이밍·폰트 fallback 미문서화도 함께 지목했으나 이번 세션 범위 밖으로 남김. → `diagnosis/external-comparison-airbnb-2026-09-06.md`

### 2026-09-06 — Text Input 컴포넌트를 다른 Figma 파일에서 복사해와 이 시스템 컨벤션으로 재작업
사용자가 다른 파일에서 만들어둔 Text Input 컴포넌트를 이 파일(`6800:8767`)에 복사해왔다("복사본"). `get_metadata`로 확인한 결과 이미 정식 `COMPONENT_SET`(26 variant)이라 별도 구조 변환은 불필요했다. Property 축 이름이 우리 규칙과 충돌(원본 `Status`=콘텐츠 채움 vs `State`=인터랙션 상태, 우리는 "상태 축=항상 Status")해 원본 `Status`→`Type`, 원본 `State`→`Status`로 교환 리네임했고, `Required`/`Counter`/`Button`은 `Show {명사}` 접두사로 통일(`Show Required Mark`/`Show Counter`/`Show Button`). Figma Plugin API의 `editComponentProperty`로 키를 리네임하면 26개 자식 레이어 이름이 자동으로 따라 바뀜을 확인 — 손으로 개별 레이어를 고칠 필요 없음. → 플랜 파일(`~/.claude/plans/figma-file-text-valiant-raven.md`)

### 2026-09-06 — Text Input 색상 195곳·텍스트 86곳을 로컬 Foundation 토큰으로 전량 재바인딩
복사본의 fill/stroke가 우리 로컬 변수(`Color/Primitive/*`)와 외부(원본 파일) 변수·스타일이 섞여 바인딩돼 있었다(같은 회색 하나가 레이어에 따라 로컬/외부로 갈리기도 함) — 기존에 이미 겪은 "원격 스타일 참조" 문제와 같은 유형. 이름으로 매칭하지 않고 **실제 렌더링된 hex/폰트 스펙으로 매칭**하는 방식을 채택(예: 원본의 `text/body/small`(15px/Regular/140%)은 이름이 비슷해 보여도 우리 `body/small`(14px)이 아니라 `body/compact`(15px)와 매칭되는 "false friend"였음). 전체 Gray/Error Primitive 스텝을 실측 조회해 미확정이던 hex 3건(`#EEEFF1`→`Gray/100` 정확 일치, `#C91D1D`→`Error/500` 정확 일치, `#FAFAFA`→정확 일치 없어 `Gray/50`로 스냅)을 전부 해소, 새 Primitive를 추가하지 않았다. 15px Bold 텍스트는 처음엔 없는 값이라 판단해 신규 스타일 생성을 검토했으나, 실측 결과 `text/heading/xxsmall`이 스펙까지 정확히 동일함을 발견해 사용자 확인 후 신규 생성 대신 재사용으로 전환 — "필요한 토큰이 없으면 임의로 새로 만들지 않고 먼저 확인을 구한다"(§0 원칙2)를 실제 발견 이후 적용한 사례. 최종적으로 `get_variable_defs` 재조회 결과 외부 참조 0건 확인. → `design-system/design.md` §2.16

### 2026-09-06 — guide text 아이콘을 raw 도형에서 정식 인스턴스로 승격, `Icon / Status / Success` 신규 생성
Text Input의 guide text(도움말) 성공 톤 아이콘이 이미 복사본 안에 파란 원+체크 벡터로 만들어져 있었지만 컴포넌트 인스턴스가 아니라 raw 도형이었다. 기존 `Icon / Status`(Error/Warning만 존재, 개별 standalone 컴포넌트로 이미 분리돼 있던 패턴)에 맞춰 이 raw 도형을 승격해 `Icon / Status / Success`(`6810:8342`)를 신규 생성하고, 에러 톤은 기존 `Icon / Status / Error`를 재사용해 guide text 아이콘 11곳 전부를 정식 인스턴스로 교체했다. Dialog·`Item Card / Cart`가 이미 고아 컴포넌트("x-01")를 잘못 참조 중인 기존 미해결 이슈(design.md §5)는 사용자 확정대로 이번 범위에 포함하지 않고 그대로 남겨뒀다. → `design-system/design.md` §2.16, `design-system/design.md` §5

### 2026-09-06 — `Masking=MaskedWithLabel`은 프로토타입 상태로 잠정 확정
원본 복사본에 `Masking=Label` 조합이 1건 있었으나 마침 `Type=Empty`(입력값 없음)라 실제 텍스트가 있을 때 어떻게 보여야 하는지 특정할 수 없었다. 사용자 확인 결과 "`Masked`(점으로 가림)와 동일하되 옆에 추가 인디케이터가 붙는다"로 확정 — 정확한 인디케이터 형태(문구·아이콘)는 프로토타입(점+회색 "라벨" pill 태그)을 만들어 스크린샷으로 보여준 뒤 잠정 승인받았다. 최종 비주얼은 §5 후속 과제로 남김. → `design-system/design.md` §2.16, §5

### 2026-09-06 — `Text Input`의 `Show Required Mark`/`Show Counter`/`Show Button`을 VARIANT에서 진짜 BOOLEAN으로 재구성
사용자가 "properties가 variant 밖에 없는데 성격상 boolean도 있어야 할 것 같다"고 지적 — 확인 결과 정확했다. `Button`의 `Show Leading Icon`/`Show Trailing Icon`은 이미 진짜 BOOLEAN(+INSTANCE_SWAP) 타입인데, `Text Input`의 세 속성은 이름만 `Show {명사}` 규칙을 따르고 실제로는 True/False별로 별도 COMPONENT가 존재하는 VARIANT 축이었다. 같은 나머지 4축(`Type`/`Status`/`Helper Text`/`Masking`) 조합 안에서 이 3개 값만 다르던 변형들을 하나로 병합(26→11개) — 병합 시 생존 변형에 없던 레이어(`*`, 카운터, 트레일링 버튼)는 다른 변형에서 복제해 보강한 뒤, `addComponentProperty`로 진짜 BOOLEAN 3개를 새로 정의하고 각 레이어의 `componentPropertyReferences.visible`을 연결했다. 트레일링 버튼을 끄면 입력창이 328px 전체 폭으로 자연스럽게 늘어나야 하는데 처음엔 그렇지 않아(부모 행의 `primaryAxisSizingMode`가 `AUTO`라 버튼이 숨으면 행 자체가 244px로 쪼그라듦), 행을 `FIXED` 328px로 고정하고 안쪽 wrapper·text box를 `FILL`로 바꿔 해결했다. 조사 중 별개로 변형 27개 중 2개가 완전히 동일한 이름·내용으로 중복돼 있어(`componentPropertyDefinitions` 읽기 자체가 에러나던 원인) 하나를 삭제 — 정확한 발생 경위는 확인하지 못했으나 지난 세션의 대량 rename 스크립트 중 하나에서 우연히 생긴 것으로 추정된다. → `design-system/design.md` §2.16(v0.35), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — Text Input 트레일링 버튼이 원본 파일의 REMOTE 컴포넌트를 참조 중이던 것을 재연결
사용자가 "Show Button에 쓰이는 버튼은 Button 컴포넌트랑 연결되어야 한다"고 지적 — 확인 결과 `getMainComponentAsync().remote === true`, 소속 세트가 우리 파일 어느 페이지에도 없는 진짜 원격 참조(`Type`/`Size`/`State`/`Layout` 축 구조가 우리 `Button`과 다름)였다. 이전에 겪은 "원격 스타일 참조"(Quantity Stepper) 문제와 같은 유형이 컴포넌트 단위로 재발한 것. 로컬 `Button`의 `Type=Primary, Size=Medium`(Default/Disabled 각각 대응)으로 `swapComponent()` 재연결 — remote 쪽도 원래 `Type=Primary`였어서 의미 그대로 보존됨. 부작용 2건 발견·수정: (1) swap 직후 우리 Button의 기본 Leading/Trailing 아이콘 placeholder가 함께 켜져 있어 `Show Leading Icon`/`Show Trailing Icon`을 꺼서 정리 (2) swap이 `componentPropertyReferences.visible`로 바인딩된 `visible` 상태를 일시적으로 false로 되돌리는 것을 발견 — 참조 자체는 안 끊겼지만 값이 초기화되는 부작용이라, swap 후 매번 visible을 재확인해야 한다는 걸 후속 작업을 위해 기록해둔다. → `design-system/design.md` §2.16(v0.36), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — Text Input의 `Type`을 `Content`로 개명 (Status와의 개념적 겹침 해소)
사용자가 `Type`(Empty/Filled/Done)과 `Status`(Default/Active/Disabled/Invalid)가 서로 겹쳐 보인다고 지적 — 확인 결과 두 축의 값 자체는 겹치지 않지만, 둘 다 "필드의 상태"처럼 읽혀서 `Type`이라는 이름이 실제 의미(콘텐츠 채움 미리보기)와 안 맞았던 게 원인이었다(이전 세션에 `State`→`Status` 교환 매핑을 하면서 남은 이름을 그냥 `Type`에 붙인 결과). `Type`을 `Content`로 개명 — 값은 그대로, 11개 자식 이름만 자동 갱신됨. `design.md §3`에 "콘텐츠 축이 상태처럼 읽히면 `Type` 대신 더 구체적인 이름을 쓴다"는 예외 규칙을 각주로 남겼다. → `design-system/design.md` §2.16·§3(v0.37), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — 트레일링 버튼 활성화 여부: 새 Boolean 대신 `Content=Done` 삭제로 정리
사용자가 트레일링 버튼의 활성/비활성도 Boolean으로 만들고 싶다고 요청 — 확인 결과 `Content=Done` 값 하나(11개 중 1곳에서만 쓰임, 화면상 `Content=Empty`와 완전히 동일하고 차이는 내부 `Button` 인스턴스의 `Status`뿐)가 그 역할을 몰래 하고 있었다. 다만 새 Boolean으로 깔끔하게 처리할 수는 없다는 기술적 제약을 확인 — Figma `componentPropertyReferences`는 `visible`/`characters`/`mainComponent`만 지원한다는 걸 2026-09-01 Item Card 작업에서 이미 한 번 확인했고, 이번에도 그대로 적용됐다(Boolean으로 중첩 컴포넌트의 Variant 선택 자체를 스위칭하는 건 API가 지원 안 함). 대신 `Content=Done` 값을 완전히 삭제하고 `Content`를 Empty/Filled 2개로 단순화(11→10개 변형) — 트레일링 버튼의 활성/비활성은 이미 로컬 `Button`으로 정상 연결돼 있으니(지난 세션 수정) 중첩 인스턴스를 직접 선택해 그 자체의 `Status`를 바꾸는, 이미 가능한 방식으로 처리하기로 함. Text Input에 새 속성을 얹지 않는 게 오히려 "Text Input의 스위치"와 "중첩 Button 자체의 Status"라는 두 개의 서로 다른 컨트롤이 헷갈리게 공존하는 걸 막는다고 판단. → `design-system/design.md` §2.16(v0.38), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — guide text 성공/에러 톤 분리 시도 실패, 기존 Status 종속 구조 유지로 결론
사용자가 guide text 아이콘의 성공/에러 톤을 Boolean이 아닌 별도 Type(Success/Error)으로 분리하고 싶다고 요청 — `Guide Type` INSTANCE_SWAP 속성을 만들어 6개 기존 아이콘 인스턴스(Invalid variant 4곳=Error, Active variant 2곳=Success)에 바인딩했더니, **여러 variant에 걸쳐 같은 컴포넌트 속성을 바인딩하면 variant별로 독립된 값을 가질 수 없고 전체가 마지막에 쓴 값 하나로 통일돼버리는 것을 발견**(처음엔 전부 Error로, 되돌리려는 시도에서도 의도한 대로 안 갈림 — Boolean의 `visible` 때와는 다른 종류의 제약). 속성 정의 자체를 삭제하고 6개 아이콘을 각자 원래 마스터(Error/Success)로 직접 재할당해 원상복구, 스크린샷으로 정상 복원 확인. 사용자와 논의 후 (1) 이 상태(Status=Active→Success, Invalid→Error 종속)를 그대로 유지하고 (2) 이 종속 관계와, 다른 조합이 필요할 때 중첩 아이콘 인스턴스를 직접 교체하면 된다는 것을 문서에 명시하는 것으로 결론지었다 — Variant 축으로 분리하는 대안도 검토했으나 실제로 한 번도 같이 안 쓰인 조합(Active+Error 등)까지 만들어야 해서 채택하지 않음. → `design-system/design.md` §2.16(v0.39), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — `Masking`을 `Input Type`(Default/Masking/Number)으로 재구성, `MaskedWithLabel` 삭제
사용자가 입력값의 종류(기본 텍스트/비밀번호/숫자)도 variant로 고를 수 있으면 좋겠다고 제안 — 기존 `Masking`(None/Masked/MaskedWithLabel)과 개념이 겹쳐, 별개 축을 추가하는 대신 `Masking`을 대체하는 `Input Type`으로 재구성하기로 사용자와 합의했다. 값 매핑: `None`→`Default`, `Masked`→`Masking`, `Number`는 신규(대표 변형을 clone해 이름만 바꿈, Figma 상 `Default`와 시각적으로 동일 — 실제 키보드 타입 차이는 구현 단계에서만 의미가 있다고 사용자 확인). 아직 최종 비주얼이 확정 안 됐던 `MaskedWithLabel`(§5 백로그이던 라벨 태그 프로토타입 포함)은 이번에 완전히 삭제 — 필요해지면 나중에 다시 만들기로 함. 결과: 9개(리네임)+1개(신규 Number) = 10개 변형, 에러 없음, 스크린샷 회귀 없음 확인. → `design-system/design.md` §2.16(v0.40), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — 누락된 기본 조합(Empty+Active, 버튼 없는 Default) 복원 + 유령 중복 3번째 재발
사용자가 `Number`를 직접 삭제한 뒤 "Content=Empty가 Disabled처럼 보인다", "Status=Default가 버튼 타입인 게 어색하다"고 지적 — 확인 결과 (1) `Content=Empty, Status=Active`(빈 칸+포커스, 실제로 가장 흔한 상태) 변형이 통째로 없어져 있었고 (2) `Status=Default`의 유일한 예시가 트레일링 버튼을 항상 포함하고 있어 "Default 상태=버튼 있음"처럼 보였다. 같은 확인 과정에서 **유령 중복 버그가 3번째로 재발**한 것도 발견(`6802:10613`=`6800:8867` 복제, `6802:10627`=`6800:9026` 복제) — 이번엔 내 스크립트가 아니라 사용자가 Figma를 직접 만지던 중에 발생, 삭제로 정리. 실측 토큰(Disabled=배경 `Gray/100`+테두리 `Gray/300`, Default=배경 없음+테두리 `Gray/300`, Active=배경 없음+테두리 `Gray/500`+파랑 커서)을 근거로 `Content=Empty, Status=Active`를 `Empty/Disabled`에서 clone해 재구성(테두리 색·커서 추가). `Status=Default`는 새 변형을 또 만드는 대신, **기존 마스터(`6800:9088`) 자체의 버튼 인스턴스 `visible`을 false로 꺼서 "버튼 없는 게 기본 모습"으로 바꾸는 쪽을 택함** — Boolean으로 이미 설계된 기능이니 새 변형을 만들면 오히려 "같은 축 조합에 이름만 다른 두 컴포넌트"라는, 이번에 계속 겪은 문제를 스스로 반복하는 셈이라 판단. 결과: 9개 변형, 에러 없음. **전체 그룹 스크린샷은 이번에도 겹쳐 보이는 캐시 지연 현상을 보였으나, 개별 노드 스크린샷은 전부 정상 — 이 컴포넌트를 검증할 땐 개별 노드 스크린샷을 기준으로 삼아야 한다**는 것도 재확인. → `design-system/design.md` §2.16·§5(v0.41), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — Helper Text=OneLine/TwoLine 톤 커플링 재확인, TwoLine은 유지하고 줄별 독립 속성으로 해결
사용자가 "OneLine=Success, TwoLine=Error로 나온다"고 지적 — 실제로는 톤이 줄 수에 종속된 게 아니라 `TwoLine` 조합이 파일에 단 하나(Invalid+Masking)뿐이라 생긴 커버리지 착시였다. `TwoLine` 삭제 여부를 사용자에게 먼저 의견만 구하는 방식으로 논의(실행 보류 요청 있었음) — 이 컴포넌트가 비밀번호 입력 규칙 안내에 주로 쓰일 예정이고, 그런 경우 조건별 부분 통과(예: "8자 이상"은 통과, "특수문자 포함"은 미통과) 표시가 실제로 필요하다는 걸 확인해 `TwoLine`은 유지하기로 결정. 사용자가 "helper text 영역에서 줄마다 error/success를 설정할 수 있어야 한다"고 요구사항을 구체화해, 유일한 `TwoLine` 예시의 두 guide 줄 각각에 `Guide Icon 1`/`Guide Icon 2`(INSTANCE_SWAP) 속성을 신설·1:1 바인딩했다 — 지난번 "하나의 속성을 여러 variant에 공유시키면 값이 하나로 통일된다"는 제약과 달리, 이번엔 **한 variant 안의 서로 다른 두 레이어에 각각 전용 속성을 바인딩**하는 것이라 정상적으로 독립 작동함을 테스트 인스턴스로 직접 확인(마스터의 실제 값에 영향 없이 테스트 인스턴스만 바뀜). 겸사겸사 placeholder 텍스트("guide text" 제네릭 문구)도 실제 비밀번호 규칙 예시로 교체했다 — 참고로 지난번에 이 예시가 "가나다"로 손상돼 있다고 봤던 건 재확인 결과 오독이었고(구조는 처음부터 온전했음), 실제로 손상된 건 없었다. → `design-system/design.md` §2.16(v0.42), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — 아이콘+텍스트를 하나로 묶은 `Guide Row` 컴포넌트 신설
사용자가 "아이콘만 따로 고르지 말고 아이콘+텍스트를 조합으로 고를 수 있게 해달라"고 요청 — 확인해보니 v0.42의 `Guide Icon 1/2`(아이콘만 INSTANCE_SWAP)는 아이콘은 바뀌어도 텍스트 색은 안 따라가는 한계가 있었다(텍스트 `fills`는 `componentPropertyReferences`로 바인딩 불가, `visible`/`characters`/`mainComponent`만 가능하다는 기존에 확인한 제약 때문). 아이콘+텍스트를 색까지 통째로 묶어 바꾸려면 그 둘을 **하나의 스왑 가능한 컴포넌트**로 만드는 것 외엔 방법이 없다고 판단해, 기존 guide 줄(아이콘+텍스트)을 그대로 복제해 신규 컴포넌트 `Guide Row`(`Icon` 페이지, `Status`=Success/Error variant + `Text` 속성)를 만들었다. `TwoLine`의 두 줄을 이 `Guide Row` 인스턴스로 교체하고, Text Input에는 `Guide Row 1`/`Guide Row 2`(INSTANCE_SWAP, mainComponent 바인딩) 속성만 새로 걸었다 — 테스트 인스턴스로 스왑 시 아이콘·텍스트 색이 항상 함께 바뀌고 마스터는 영향받지 않음을 확인. **다만 문구(텍스트 내용) 자체는 Text Input 레벨 속성으로 노출하지 못했다** — Figma가 인스턴스 하위 레이어(중첩 인스턴스 안의 텍스트)에는 컴포넌트 속성 참조를 걸 수 없다는 제약(`in set_componentPropertyReferences: Cannot set component property references on instance sublayer` 에러로 확인) 때문. 문구를 바꾸려면 중첩된 `Guide Row` 인스턴스를 직접 선택해 그 자체의 `Text` 속성을 편집해야 한다 — 톤 선택(Text Input 레벨)과 문구 편집(중첩 인스턴스 레벨)이 2단계로 나뉘는 것을 감수하기로 함. → `design-system/design.md` §2.16(v0.43), `diagnosis/text-input-integration-2026-09-06.md`

### 2026-09-06 — Warning Primitive 11단계 색상 전면 교체(노랑-골드 → 주황 계열)
사용자가 Foundation 페이지에 직접 그려둔 새 참고 팔레트를 근거로 `Warning` 계열 색상 교체를 요청. `Color/Primitive/Warning/50~1000` 11개 변수의 값만 교체(변수·별칭 구조는 그대로 유지) — `Feedback/Warning`(Warning/400 alias)/`Feedback/Warning Background`(Warning/50 alias)/`Feedback/Warning Text`(Warning/600 alias) 3개 Semantic 토큰은 별도 작업 없이 자동으로 새 값을 반영했다. `Message Box`(Status=Warning) 스크린샷으로 확인. Foundation 문서 페이지의 스와치는 변수 바인딩이라 자동 갱신됐지만, 그 옆 hex 텍스트 라벨 11개는 예상대로 이전 값에 멈춰 있어(2026-09-01 Success 라벨 버그와 동일 유형, 반복 발생 패턴) 수동으로 갱신. 사용자가 그려둔 참고 스와치는 반영 후 삭제. → `diagnosis/color-foundation-history.md`, `design-system/design.md` §1.1

### 2026-09-06 — Message Box 전용 Error Text/Warning Text alias 재조정
사용자 요청으로 `Feedback/Error Text`를 Error/600(`#9c1616`)→Error/500(`#c91d1d`)로, `Feedback/Warning Text`를 Warning/600(`#d68200`)→Warning/700(`#a35d00`)로 alias 대상만 변경(Figma 실행, `setValueForMode`로 alias 포인터만 교체). 두 토큰 모두 `design.md §1.1`에 이미 "`Message Box` 전용 배경·텍스트 페어"로 문서화돼 있어 다른 컴포넌트에 영향이 없음을 사전에 확인 — `Message Box` Status=Error/Warning 스크린샷으로 최종 확인. → `design-system/design.md` §1.1(v0.45)

### 2026-09-06 — 신규 컴포넌트 `Text Area` 등록 (상품 리뷰 작성용 멀티라인 입력)
사용자가 상품 리뷰 작성처럼 많은 텍스트를 입력하는 화면에 대응할 컴포넌트가 필요하다고 요청, 실제 사용 예시 화면(`6801:4770`, "review-writing-screen")을 참고 자료로 제공했다. 두 가지를 먼저 확인: (1) `Text Input`의 변형이 아니라 별개 컴포넌트로 만들지(→ 별개, 테두리 유무·전체 폭 사용 등 구조가 근본적으로 다름) (2) 상태 범위를 참고 화면에 나온 것(Empty만)으로 한정할지 `Text Input`처럼 전체로 만들지(→ 전체, 일관된 품질 위해).

참고 화면의 `review-textarea-container`(`6801:4794`)를 실측한 결과: 테두리·배경 없음(완전 borderless), placeholder는 `Text/Description`(Gray/600) 색상의 `body/small`(14px/140%) 스타일, 좌우 패딩 없이 화면 마진만으로 위치. 이 실측값을 그대로 재사용해 `Content`(Empty/Filled) × `Status`(Default/Active/Disabled/Invalid) + `Show Counter`(Boolean) 조합의 6개 변형을 신규 제작(Figma 실행):
- Active: 텍스트 앞 커서(Text Input과 동일 caret) 추가
- Disabled: 테두리가 없어 `Text Input`처럼 테두리+배경을 함께 바꾸는 방식을 못 써서, `Gray/50` 배경 워시 하나로만 비활성 표현(패딩·radius는 안 줘서 다른 Status와 정렬이 어긋나지 않게 함)
- Invalid: 하단에 기존 `Guide Row`(Error) 컴포넌트를 그대로 재사용해 안내 문구 표시
- Show Counter: `Filled+Active` 마스터 하나에만 카운터 레이어를 두고 Boolean으로 visible 바인딩(Text Input의 Show Button과 동일한 원칙 — 처음부터 모든 조합에 카운터를 넣지 않고 대표 변형 하나에만 둠)

전 과정에서 지난 세션에 겪은 실수를 피하려고 신경씀: Filled+Active를 카운터 유무로 별도 변형 2개 만들려다 "같은 축 조합에 이름만 다른 컴포넌트가 생기는" 문제를 스스로 알아채고 즉시 하나로 병합 후 Boolean으로 전환. `Guide Row`의 `Text` 속성 설정 직후 스크린샷이 구값을 보여준 것도 실제로는 정상 반영된 상태였음을 재조회로 확인(캐시 지연 패턴, 기존에 이미 확인된 것과 동일). → `design-system/design.md` §2.17(v0.46), `diagnosis/text-area-component-2026-09-06.md`

### 2026-09-07 — `Bottom Sheet / Informational`의 "Discription" 오탈자 수정
`design.md §3`에 이미 이 오탈자를 반면교사로 든 각주가 있었으나(새 이름 추가 전 철자 재확인 권장 규칙의 근거) 실제 Figma 속성 이름 자체는 그대로 남아있었음을 포트폴리오 정리 중 재확인해 발견. `editComponentProperty`로 `Discription` → `Description`으로 정정(7개 variant 이름 자동 반영, 값 변경 없음, 에러 0건). → Figma `Bottom Sheet / Informational`(`4043:2686`)

### 2026-09-07 — 신규 화면 "마이페이지" 제작 — 새 컴포넌트 0개, 전량 기존 자산 재사용
아직 Figma에 없던 마이페이지 화면을 design.md만 근거로 신규 제작(`Ⓝ Note` 페이지, `📱 마이페이지 (design.md 검증)`). §6 Screen Composition Patterns의 "패턴 2(스크롤형 화면)" 리듬(8px 섹션 구분/1px 아이템 구분)을 그대로 적용. 사용한 기존 컴포넌트: `App Bar`(Type=Title), `Text Button`, `Thumbnail`(xsmall, radius를 `full` 토큰으로 override해 원형 아바타로 구성), `Section Header`, `List`(type=Image·주문내역용 / type=Default·메뉴용), `Bottom Navigation`+`Bottom Navigation Item`(Icon=Mypage를 `swapComponent()`로 State=On 전환). 색상·spacing 전부 Variable 바인딩(`setBoundVariable`), 하드코딩 없음.

새 컴포넌트는 만들지 않음 — 대신 다음 3가지는 Foundation 토큰만으로 조합(컴포지션)해 해결: (1) 주문/구매 퀵메뉴 4열 행 — `Icon / Utility`가 Filter/Star/Default 3종뿐이라 실제 메뉴 의미와 안 맞아 아이콘 없이 텍스트+세로 divider만으로 구성 (2) 주문 상태 요약 4단계 — `Tab Group`은 "같은 데이터를 다른 관점으로 재구성"할 때 전용(§4 원칙 10)이라 순수 상태 표시엔 role이 안 맞아 재사용하지 않고, 숫자+라벨 텍스트 조합으로 신규 구성 (3) 혜택(쿠폰/포인트) 3열 통계 — 대응 컴포넌트 없어 동일한 텍스트 조합 패턴 재사용. `List`의 text-only 메뉴 행은 Thumbnail 인스턴스를 `visible=false`로 꺼서 사용(마스터가 Default 타입에도 썸네일을 보유하고 있음을 이번에 확인 — design.md 서술과 실제가 다른 기존 사실, 이번 범위에서 design.md는 수정하지 않음). "Info Row" 컴포넌트는 status.md 등 문서에 언급이 있었으나 실제 Figma엔 존재하지 않음을 이번에 확인(재사용 시도 전 검증으로 발견). → Figma `Ⓝ Note` 페이지

### 2026-09-08 — `Secondary` Primitive에 `50` 단계 신규 추가
`Primary`/`Gray`는 이미 `50` 단계(가장 밝은 톤)를 갖고 있는데 `Secondary`만 `100`부터 시작해 패밀리 간 단계 수가 불일치했음을 사용자가 지적, `Color/Primitive/Secondary/50`(`#f2f9fd`) 변수를 신규 생성(scopes=`ALL_SCOPES`, 기존 `Secondary/100` 등과 동일 컨벤션)하고 Foundation "🎨 Color" 문서 페이지의 `Secondary` 스와치 행 맨 앞(100 앞)에 추가. 다른 9개 스와치는 기존 그대로, 새 스와치만 실제 변수에 바인딩. → Figma "🎨 Color" 페이지, `VariableID:7085:2`

### 2026-09-08 — 신규 화면 "상품 상세 페이지" 제작 — PRD 요구사항을 실제 참고 화면보다 우선
화면 생성 전에 목적·정보구조(Must/Should/Could/제외)·정보밀도 판단을 먼저 텍스트로 정리한 뒤 제작(`Ⓝ Note` 페이지, `🛍️ 상품 상세 (design.md 검증)`). 실제 참고 화면(`6249:8189`)이 있었지만, PRD(`prd-v0.1.md`) P2(정보 분산)·P3(판단 근거 불명확) 문제를 그 화면 자체가 그대로 갖고 있어(앵커 내비게이션 부재, 맥락 없는 절대 수치 노출 등) 이번엔 참고 화면을 그대로 따르지 않고 PRD의 Scenario 2/FR-D 요구사항을 우선했다 — design.md §0 "컴포지션 우선" 원칙과 충돌하는 지점이라 인지하고 진행, 근거는 `diagnosis/`가 아니라 이 항목에 직접 기록. 새 컴포넌트는 만들지 않음(Price Block/Spec Row/Rating Display/Review Summary/Review Card/Item Card Recommendation/App Bar 2종/Section Header/Chip/Option Chips/Quantity Stepper/Button/Action Bar 전부 기존 인스턴스). `anchor-nav`, `ai-review` 2곳은 대응 컴포넌트가 없어 "미정의 조합"으로 이름 붙여 신규 컴포넌트가 아님을 명시. → Figma `Ⓝ Note` 페이지

### 2026-09-08 — Divider 색상 규칙 재정정 — Semantic이 아니라 두께별 Primitive 직접 참조가 실제 패턴이었음
직전에 사용자 요청으로 `Background/Divider` Semantic 토큰 자체를 `Gray/200`→`Gray/100`으로 바꿨었는데, 사용자가 "내가 말한 적 없는 규칙이 적용된 것 같다"며 실제 화면(`★ 장바구니`, `6764:3802`) 기준으로 다시 정의해달라고 요청. divider 14곳을 전수 재조회한 결과 **1px divider는 전부 `Color/Primitive/Gray/200`, 8px divider만 `Color/Primitive/Gray/100`**을 쓰고 있었고, 둘 다 `Background/Divider` Semantic을 거치지 않고 Primitive를 직접 바인딩하고 있었다(Semantic 토큰은 Dialog/Bottom Sheet 내부 스크롤 구분선 등 컴포넌트 내부에서만 실사용). `Background/Divider`는 `Gray/200`으로 원복하고, 그동안 만든 8px divider 6곳(장바구니 2·상품 상세 4)만 `Gray/100` Primitive에 직접 재바인딩(Figma 실행). design.md §6에 두께별 색상 규칙과 바인딩 경로 차이를 명문화(v0.47). → `design-system/design.md` §6

### 2026-09-08 — `Option Chips / Size`의 `Status=Default` 마스터에서 실제 컴포넌트 버그 발견·수정
상품 상세 페이지의 옵션 선택 Chip 2개가 `Selected` 값과 무관하게 둘 다 파란 테두리로 보인다는 사용자 지적으로 조사. `Selected` Boolean 자체는 `1kg`=true/`2kg`=false로 정확히 설정돼 있었는데도 둘 다 파랑으로 보였음 — 원인은 `Status=Default` 마스터(`6373:23546`)의 **최상위 인스턴스 프레임 자신이 파란 stroke를 직접 갖고 있어서**였다. 내부에는 `Selected` Boolean에 제대로 바인딩된 `selected-border`(파랑) 레이어가 이미 정상적으로 존재했지만, 2026-09-03에 Chip에서 이미 한 번 발견했던 것과 똑같은 렌더링 규칙("부모 자신의 stroke는 항상 자식보다 위에 그려진다")에 걸려 내부 레이어의 on/off와 무관하게 항상 파란 테두리로 보였다. `low_stock`/`sold_out` 형제 variant는 이 문제가 없었음(마스터 stroke 자체가 없음) — `Status=Default`에만 있던 개별 실수. 마스터의 `strokes`를 빈 배열로 정리해 해결(Figma 실행, `Option Chips / Size` 전체에 영향 — 이 컴포넌트를 쓰는 모든 화면에 소급 적용됨). → Figma `Option Chips / Size`(`6373:23548`) `Status=Default`(`6373:23546`)

### 2026-09-08 — `spacing` 여백 프레임 색상 정정 — `Background/Background`(Gray/50) → `Static/0`(흰색)
상품 상세 페이지에서 순수 여백(`spacing`) 프레임을 8px divider와 시각적으로 구분하기 어렵다는 사용자 지적. 원인은 `spacing` 프레임이 `Color/Semantic/Background/Background`(Gray/50, `#f4f6f6`)에 바인딩돼 있어 바로 옆 8px divider(`Gray/100`, `#eeeff1`)와 명도차가 거의 없었기 때문 — 장바구니 화면을 만들 때부터 반복해온 실수(당시엔 발견되지 않음). 상품 상세 페이지의 `spacing` 프레임 12곳 전부 `Color/Primitive/Static/0`(순백)로 재바인딩(Figma 실행). design.md §6 공통 규칙에 "여백은 흰색, divider만 회색" 규칙 명문화(v0.48) — 장바구니 화면은 이번 범위에 포함하지 않았으므로 같은 문제가 남아있을 수 있음, 필요 시 별도 확인. → `design-system/design.md` §6

### 2026-09-08 — `Item Card / Recommendation` 상품명 `textAlignVertical` 버그 발견·수정 — 마스터 레벨 수정으로 전체 화면에 소급 적용
사용자가 상품 상세 구버전 화면(`🛍️ 상품 상세 (design.md 검증)`, `7100:5950`, 안의 `7100:6017` 캐러셀)을 가리키며 상품명이 1줄/2줄로 갈릴 때 카드 간 텍스트 위치가 들쑥날쑥해 보인다고 지적, 수정 여부와 규칙화 여부를 물었다. 조사 결과 Price Block 등 하위 요소의 절대 Y좌표는 모든 카드에서 이미 동일했다(상품명 박스 자체가 2줄 분량 고정 높이 40px). 실제 원인은 상품명 텍스트가 `textAlignVertical: CENTER`였던 것 — 2줄 상품명은 박스를 꽉 채우지만 1줄 상품명은 박스 중앙에 떠서 시작선이 아래로 처지는 시각 효과였다. `Item Card / Recommendation` 컴포넌트 셋(`6253:1706`)의 `Type`(Default/Button) × `Discount`(Applied/None) 4개 마스터(`6051:644`, `6264:1396`, `6253:1678`, `6264:1411`) 전부 상품명 텍스트를 `TOP`으로 일괄 수정(Figma 실행) — 컴포넌트 마스터 레벨 수정이라 이 컴포넌트가 쓰이는 모든 화면(장바구니 2종, 상품 상세 2종)에 자동 소급 적용됨, 화면별 개별 수정 불필요. `Item Card / Cart`(`6067:825`)도 같은 패턴이 있는지 확인했으나 상품명 박스가 애초에 1줄 고정(높이 22px)이라 구조적으로 해당 없어 미수정. design.md §4 원칙 11에 "고정 줄 수 예약 박스는 TOP 앵커링" 일반 규칙으로 편입, §2.8에도 근거 추가(v0.49).

부수 발견: 사용자가 링크한 `7100:5950`은 지난 라운드에 "기존 UI를 수정하지 말고 새 frame을 만들라"는 지시로 새로 만든 `7112:1864`("… v2")와는 별개의, 그 이후로 손대지 않은 이전 프레임이다 — 이번 마스터 레벨 수정은 컴포넌트 자체를 고친 것이라 두 프레임 모두에 반영되지만, 최신 콘텐츠(결제 혜택 블록, 3번째 리뷰 카드 등)는 v2에만 있다. 두 프레임을 계속 나란히 둘지 정리할지는 사용자에게 확인 필요. → Figma `Item Card / Recommendation`(`6253:1706`), `design-system/design.md` §2.8·§4

### 2026-09-08 — 위 규칙 보강: "박스 높이도 FIXED여야 한다"는 조건 추가 (문서만, Figma 변경 없음)
바로 위 항목에서 `textAlignVertical: TOP`만 규칙화했는데, 사용자가 "상품명의 길이가 다를 경우, 텍스트 길이만큼의 영역이 확보되는 게 아니야"라며 새 대조 예시(`7100:14177`)를 직접 만들어 제시 — 3장의 카드 중 1·3번째는 정상(마스터 그대로, `FIXED` 40px), 가운데 1장만 컴포넌트에서 분리(detach)한 뒤 상품명 텍스트를 수동으로 짧게 줄이고 박스를 `HUG`(실제 텍스트만큼만 높이 확보)로 바꿔둔 상태였다. 스크린샷으로 확인한 결과 가운데 카드는 상품명이 "도브 화이트피치 리밸런"에서 그대로 잘려 둘째 줄 없이 끝나 있었고, 그 아래 가격·배송 정보 전체가 20px 위로 당겨 올라와 카드 전체 높이가 옆 카드보다 짧아져 있었다(294px→274px) — 텍스트 정렬(TOP/CENTER)과는 별개로, 박스 자체의 높이가 내용에 따라 늘었다 줄었다 하면(`HUG`) 그것만으로도 들쑥날쑥함이 생긴다는 걸 보여주는 예시였다.

실제 컴포넌트가 이 문제를 갖고 있는지 파일 전체의 살아있는 인스턴스(`INSTANCE` 타입) 33개를 전수조사했으나, 전부 이미 `layoutSizingVertical: FIXED`(40px)였다 — 사용자가 만든 반례는 컴포넌트에서 분리된 detached 프레임이라 실제 버그가 아니라 "이렇게 하면 안 된다"는 시연용이었던 것으로 판단, Figma 실물은 수정하지 않았다. 대신 design.md §4 원칙 11과 §2.8을 "①박스는 내용과 무관하게 FIXED 높이 ②그 안에서 TOP 정렬" 두 조건으로 재작성해 v0.49의 규칙을 완성했다(v0.50). → `design-system/design.md` §4 원칙 11, §2.8

### 2026-09-09 — `Item Card` 색상 "역할별 그룹" 규칙 도입 — 실사용 매핑표 조사 후 3가지 확정 반영
사용자가 실제 커머스 카드(쿠팡 추정) 관찰로 "색은 요소가 아니라 역할별로 그룹 적용된다"는 규칙(조건부 가격=빨강, 긴급=빨강, 획득/달성=초록, 혜택 조건=연파랑 배지, 평가=주황, 나머지=무채색)을 제시. Plan Mode에서 먼저 의견만 요청받아 실제 Figma 색상을 읽기 전용으로 전수 재조회한 결과, 이 규칙 대부분(조건부 가격의 그룹 단위 적용, 긴급, 획득/달성)은 이미 실제 컴포넌트(`Price Block`, `Order Deadline`, `Spec Row`)에 지켜지고 있음을 확인했다. 이어서 실사용 260개 인스턴스를 전수 스캔해 매핑표를 만드는 과정에서 진짜 공백·불일치 두 가지를 발견했다:
1. 상품명이 `Semantic Text/Primary`가 아니라 raw `Primitive Primary/900`에 직접 바인딩돼 있었고, `Text/Primary` 토큰 자체도 그동안 엉뚱한 값(`Primary/1000`, 실사용 0건 상태로 방치)을 가리키고 있었다.
2. "무료배송"/"무료반품" 배지가 마스터-실사용 간 색이 다르게 렌더링되고 있었다(`Label` 컴포넌트 자체에는 이미 pill 배경 구조가 있었으나 `Return`만 배경색이 `Shipping`과 달랐다).

사용자가 이 조사 결과에 3가지로 확정 답변: (1) 상품명·`Text/Primary` = `Gray/900` (2) 무료배송 배지 text=`Gray/900`, bg=`Secondary/200` (3) `Spec Row`에 "재고 경고"·"쿠폰 할인 적용됨" 추가(단 "특가 진행중"은 제외). 이어서 Spec Row 신규 2종의 세부 스펙(재고 경고="재고 N개 남음", pill 없는 13px 빨간 텍스트 / 쿠폰 할인 적용됨=`Order Deadline Type=Default`와 동일 스펙의 아이콘+텍스트, 아이콘은 `Icon / Placeholder Square` 임시 사용, `Item Card / Cart` 전용)까지 직접 확정.

**실행(Figma)**: `Text/Primary`(`VariableID:2226:8780`) alias를 `Gray/900`으로 정정 → 상품명이 쓰이는 7개 마스터(`Item Card / Grid`, `/Recommendation` 4 variant, `/Cart` 2 variant, 지난번 텍스트 정렬 버그를 고쳤던 바로 그 마스터들) 전체를 이 Semantic 토큰에 재바인딩 → `Label`의 `Type=Shipping`·`Type=Return` 텍스트를 `Gray/900`으로, `Return`의 배경을 `Secondary/200`으로 통일 → `Spec Row` 컴포넌트 셋(`6041:519`)에 `Type=LowStock`, `Type=CouponApplied` 2개 variant를 신규 생성해 `combineAsVariants` 구조에 편입(에러 없이 `componentPropertyDefinitions`에 8개 옵션으로 정상 반영 확인). 전부 마스터 레벨 수정이라 실사용 화면(장바구니 등)에 자동 전파됨을 스크린샷으로 확인.

**부수 발견(이번 범위 밖, §5 백로그 기록만)**: 같은 `Label` 셋의 `Type=Promotion`(특가진행중)도 마스터-인스턴스 색상 드리프트가 있었고(`Type=RepeatPurchase`도 텍스트가 여전히 `Primary/900`), 사용자가 "특가 진행중은 이번엔 제외"라고 명시해 손대지 않았다. → `design-system/design.md` v0.51, §1.1, §2.8, §2.9, §5

### 2026-09-09 — 색상 바인딩 전수 재확인 중 실행 실수 발견·정정 + Rating Display 별 아이콘 신규 토큰화
사용자가 "이걸 디자인 시스템에 어떤 식으로 반영했지?"라는 질문에 "색상은 v0.51로 반영했지만 강도(굵기)는 아직 문서화 안 됨"이라고 답했더니, "색상도 바인딩시켜야 할 것 같은데"라고 재지적. Item Card 텍스트·아이콘 색상 바인딩을 전수 재확인한 결과 두 가지를 발견했다:

1. **실행 실수**: v0.51에서 만든 `Spec Row` `CouponApplied` 텍스트가 의도(`Error/400`)와 다르게 `Text/Primary`(당시 막 `Gray/900`으로 정정한 값)에 바인딩돼 있었다 — 두 색 모두 어두운 계열이라 스크린샷만으로는 구분이 안 됐고, `boundVariables`를 직접 열어봐서야 발견했다. 즉시 `Error/400`으로 정정.
2. **미해결 공백**: `Rating Display` 별 아이콘이 여전히 raw hex(`#ff9c5c`)였다. 기존 `Point`/`Warning` 11단계 중 정확히 일치하는 값이 없어(가장 가까운 `Warning/500`도 R/G/B가 미세하게 다름), 사용자에게 4가지 선택지(가까운 기존 토큰에 흡수/새 Primitive만 추가/새 Semantic까지 승격/보류)를 제시했고, **"별 아이콘 색상을 별도로 등록. 시맨틱 컬러에 등록"**을 선택했다.

**실행(Figma)**: `Color/Primitive/Rating/Star`(`VariableID:7187:2`, `#ff9c5c` 그대로, `scopes=[]`)를 신규 생성하고, `Color/Semantic/Rating/Star`(`VariableID:7187:3`, `scopes=["SHAPE_FILL"]`)가 이를 alias하도록 만들어 이 Semantic 토큰을 별 아이콘에 바인딩했다. `Rocket`(도메인 특화 개념이라 Primitive에만 머문 선례, 2026-09-04 결정)과는 다른 선택인데, 이번엔 사용자가 명시적으로 Semantic 승격을 원해 그대로 따랐다 — 이 파일의 "Semantic은 전량 Primitive를 alias" 원칙(하드코딩 금지)은 그대로 유지했다. 색상값 자체는 안 바꿨으므로 스크린샷상 시각적 변화는 없음을 확인했다. → `design-system/design.md` v0.52, §1.1, §2.8

### 2026-09-09 — Item Card 색상 역할 4종(가격/긴급/배송/혜택)을 Semantic 토큰으로 승격
사용자가 v0.51 매핑표를 다시 요청하며 자신이 정리한 `text/price`·`text/urgent`·`text/benefit`·`text/delivery`·`text/default` 5개 역할 표를 보여줬다. 실제 현황을 확인해보니 `text/default`(=`Text/Primary`)만 Semantic을 거치고 나머지 4개는 전부 `Error/400`·`Success/600`·`Secondary/200` Primitive에 직접 바인딩돼 있었다. 사용자가 "직접 참조 없이 시맨틱을 참조하는 게 이상적이지?"라고 묻길래, 이 파일이 이미 갖고 있던 판단 기준("Primitive 이름 자체가 의미를 못 담을 때만 Semantic을 씌운다" — Divider 두께별 예외·Rocket Badge 예외에서 확인된 원칙)에 비춰 이 4개 역할은 이름 없는 Primitive를 그대로 쓰고 있어 Semantic화가 맞다고 답했다. 사용자가 "시맨틱화 시켜줘, 확인 필요하면 물어보고 반영해줘"라고 승인.

Plan Mode에서 AskUserQuestion으로 두 갈래를 확인했다: (1) 혜택 라벨 텍스트(현재 `Gray/900`, 상품명과 동일값) — 신규 `Text/Benefit`을 만들지, 기존 `Text/Primary`를 재사용할지 → **재사용** 선택(값이 진짜 같고 "강조 아닌 기본"이라는 역할이라 분리 이유가 약함) (2) 배지 배경색도 범위에 포함할지 → **포함** 선택.

**실행(Figma)**: `Color` 컬렉션에 Semantic 5개 신규 생성 — `Text/Price`·`Background/Price`·`Text/Urgent`(전부 `Error/400` alias)·`Text/Delivery`(`Success/600` alias)·`Background/Benefit`(`Secondary/200` alias). `Text/Price`와 `Text/Urgent`는 오늘 값이 같지만(둘 다 `Error/400`) "조건부 가격"과 "긴급"이라는 역할이 달라 나중에 독립적으로 갈라질 수 있도록 의도적으로 분리했다(사용자 원안 그대로 따름). 이어서 14곳을 재바인딩: Price Block(`6036:452`)의 최종가·단위가→`Text/Price`, 할인율 배지 배경→`Background/Price`, 배지 텍스트→기존 `Text/On Color`(재사용) / Order Deadline(`6067:619`) 텍스트→`Text/Urgent` / Spec Row의 `LowStock`→`Text/Urgent`, `CouponApplied`→`Text/Price`, 배송 도착 텍스트 4곳(`Tomorrow`/`Fresh`/`Seller`/`Free`)→`Text/Delivery` / Label `Shipping`·`Return`의 배경→`Background/Benefit`, 텍스트→기존 `Text/Primary`(재사용). 전부 마스터 레벨이라 실사용 화면에 자동 전파됐고, 색상값 자체는 그대로 옮겼으므로 스크린샷 비교 결과 시각적 차이는 없음을 확인했다. → `design-system/design.md` v0.53, §1.1, §2.8, §2.9

### 2026-09-10 — Primitive 색상 패밀리 5개 리네임 — Primary/Secondary/Error/Warning/Success → Blue/SkyBlue/Red/Orange/Green
직전 작업(`Text/Price`, `Text/Urgent`)에서 둘 다 `Error/400`이라는 같은 Primitive를 alias하는 걸 보고, 사용자가 "Primitive 색상 패밀리 네이밍이 지금 상태로 괜찮은지" 의견을 물었다. 실제 Primitive 패밀리 10개(`Gray`/`Primary`/`Secondary`/`Point`/`Error`/`Warning`/`Success`/`Static`/`Rocket`/`Rating`)와 Foundation 문서 페이지 라벨을 재조회해, `Primary`/`Secondary`/`Error`/`Warning`/`Success` 5개가 색상값이 아니라 역할(role) 이름을 갖고 있다는 게 문제의 핵심이라고 답했다 — `Error/400`이 실제로는 "가격 강조"와 "긴급"이라는 에러와 무관한 두 의미로 이미 재사용되고 있는 게 그 증거였다. "리네임(엄격한 3계층)"과 "현행 유지+원칙 명문화" 두 방향을 제시하며 비용 때문에 후자를 추천했으나, 사용자가 "1번이 나은 것 같아. 시맨틱 컬러면 몰라도 프라이머리 컬러 네임에 error가 들어가는 게 어색해"라며 리네임을 확정했다.

**실측으로 확정한 새 이름**(대표 hex로 색상 판별): `Error`(`#e23636`)→`Red`, `Warning`(`#ffb33d`)→`Orange`, `Success`(`#36e236`)→`Green`, `Primary`(`#106def`)→`Blue`, `Secondary`(`#52aee0`, Primary보다 채도 낮은 하늘색)→`SkyBlue`. `Point`(빨강·주황 사이 버밀리언, `#f2460d`)는 사용자가 지적한 대상이 아니고 애초에 명백한 역할 이름도 아니라 이번 범위에서 제외했다. `Gray`/`Static`/`Rocket`/`Rating`은 이미 문제없는 이름이라 그대로 뒀다.

**실행(Figma)**: 5개 패밀리 55개 변수를 `setName`으로 리네임(`Color/Primitive/Error/*` → `Color/Primitive/Red/*` 등, hex 값은 안 건드림). Figma 변수 참조는 ID 기반이라 기존 Semantic 29개의 alias·컴포넌트 바인딩이 전부 그대로 유지됨을 재조회로 확인했다(끊어진 alias 0건). Foundation "⓪ Foundation" → "Color · Primitive" 프레임의 섹션 라벨 텍스트 5곳도 동일하게 갱신, 스크린샷으로 스와치 색상 자체는 안 바뀌었음을 확인했다.

**문서화 원칙**: `Text/Primary`·`State/Primary`·`Emphasis/Primary`·`Feedback/Error` 같은 **Semantic 토큰 이름 자체는 그대로 뒀다** — 이 이름들은 원래도 "역할"을 뜻하는 게 맞고, 문제는 어디까지나 Primitive 레벨이었다. design.md에서도 "슬래시+숫자"(`Primary/900`처럼 실제 Primitive 참조)만 새 이름으로 바꾸고, "이전엔 ~였다"는 과거 서술과 v0.53 이전 changelog 항목은 그 시점의 정확한 이름 그대로 남겨 역사적 기록을 보존했다. → `design-system/design.md` v0.54, §1.1
